...
router.post("/usuarios", requireRole("admin"), async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      await AuditService.registrar(req, "usuario_criado", "usuario", criado[0]?.id, undefined, { tipo: criado[0]?.tipo, ativo: criado[0]?.ativo });
      res.status(201).json({
        usuario: criado[0],
...
router.put("/usuarios/:id", requireRole("admin"), async (req: Request, res: Response) => {
  try {
    ...
    if (tipo !== undefined) {
      const tipoSolicitado = String(tipo);
      const mesmoTipo = tipoSolicitado === String(existente[0].tipo);
      const permitido = req.usuario?.tipo === "dev"
        ? ["cliente", "vendedor", "admin", "dev"].includes(tipoSolicitado)
        : mesmoTipo || ["cliente", "vendedor"].includes(tipoSolicitado);
      if (!permitido) return res.status(403).json({ erro: "Você não pode atribuir este nível de acesso" });
      atualizacoes.tipo = tipoSolicitado as any;
      revogarSessoes = tipoSolicitado !== String(existente[0].tipo);
    }
    ...
    try {
      ...
      await AuditService.registrar(req, "usuario_atualizado", "usuario", id, { tipo: existente[0]?.tipo, ativo: existente[0]?.ativo }, { tipo: atualizado[0]?.tipo, ativo: atualizado[0]?.ativo });
...
router.patch("/usuarios/:id/status", requireRole("admin"), async (req: Request, res: Response) => {
  try {
    ...
    await AuditService.registrar(req, ativo ? "usuario_reativado" : "usuario_desativado", "usuario", id, { tipo: alvo.tipo, ativo: !ativo }, { tipo: atualizado[0]?.tipo, ativo });
    res.json({ usuario: atualizado[0] });
  } catch (error: any) {
...
router.delete("/usuarios/:id", requireRole("admin"), async (req: Request, res: Response) => {
  const { id } = req.params;
  if (!req.usuario) return res.status(401).json({ erro: "Não autenticado" });
  if (req.usuario.id === id) return res.status(400).json({ erro: "Você não pode excluir o próprio acesso" });

  try {
    const alvo = (await db.select(CAMPOS_PUBLICOS_USUARIO).from(usuarios).where(eq(usuarios.id, id)).limit(1))[0];
    if (!alvo || (alvo.tipo === "dev" && req.usuario.tipo !== "dev")) {
      return res.status(404).json({ erro: "Usuário não encontrado" });
    }

    try {
      const removido = (await db.delete(usuarios).where(eq(usuarios.id, id)).returning({ id: usuarios.id }))[0];
      if (!removido) return res.status(404).json({ erro: "Usuário não encontrado" });
      await AuditService.registrar(req, "usuario_excluido", "usuario", id, { tipo: alvo.tipo, ativo: alvo.ativo }, { removido: true });
      return res.json({ modo: "excluido", mensagem: "Cadastro excluído." });
    } catch (error: any) {
      // 23503 = existem vínculos históricos protegidos por chave estrangeira.
      // Nessa situação preservamos contratos, reservas, pagamentos e auditoria
      // e arquivamos o acesso em vez de destruir o histórico.
      if (error?.code !== "23503") throw error;
      const arquivado = (await db.update(usuarios).set({
        ativo: false,
        session_version: sql`COALESCE(${usuarios.session_version}, 1) + 1`,
        atualizado_em: new Date(),
      }).where(eq(usuarios.id, id)).returning(CAMPOS_PUBLICOS_USUARIO))[0];
      await AuditService.registrar(req, "usuario_arquivado", "usuario", id, { tipo: alvo.tipo, ativo: alvo.ativo }, { tipo: arquivado?.tipo, ativo: false });
      if (alvo.tipo === "cliente") {
        await registrarHistoricoCliente(id, "cadastro_arquivado", "Cadastro arquivado", "Acesso desativado para preservar o histórico existente.", req.usuario.id);
      }
      return res.json({ modo: "arquivado", usuario: arquivado, mensagem: "Este cadastro possui histórico e foi arquivado com segurança." });
    }
  } catch (error: any) {
    console.error("[ADMIN] Erro ao excluir/arquivar usuário:", error);
    return res.status(500).json({ erro: "Não foi possível excluir ou arquivar este cadastro" });
  }
});
...
router.get("/dev/equipe", requireRole("admin"), async (req: Request, res: Response) => {
  const solicitanteDev = req.usuario?.tipo === "dev";
  const tiposEquipe = solicitanteDev ? ["dev", "admin", "vendedor"] : ["admin", "vendedor"];
  const equipe = await db.select(CAMPOS_PUBLICOS_USUARIO).from(usuarios).where(inArray(usuarios.tipo, tiposEquipe as any)).orderBy(desc(usuarios.criado_em));
  const convitesBase = await db.select().from(convitesAcesso).orderBy(desc(convitesAcesso.criado_em)).limit(200);
  const convites = solicitanteDev ? convitesBase : convitesBase.filter((convite) => convite.papel !== "dev");
...
router.patch("/dev/convites/:id/revogar", requireRole("admin"), async (req: Request, res: Response) => {
  const existente = (await db.select().from(convitesAcesso).where(eq(convitesAcesso.id, req.params.id)).limit(1))[0];
  if (!existente || (existente.papel === "dev" && req.usuario?.tipo !== "dev")) return res.status(404).json({ erro: "Convite não encontrado" });
