...
router.get("/lotes/:loteId", async (req: Request, res: Response) => {
  try { return res.json(await HospedagemService.obterMapa(req.params.loteId, String(req.query.evento_periodo_id || req.query.periodo_id || "") || null)); }
...
router.post("/lotes/:loteId/reconciliar", async (req: Request, res: Response) => {
  try { return res.status(409).json(await HospedagemService.reconciliarLote(req.params.loteId, req.usuario!.id, String(req.query.evento_periodo_id || req.query.periodo_id || "") || null)); }
...
router.post("/lotes/:loteId/quartos/lote", async (req: Request, res: Response) => {
  try { return res.status(201).json(await HospedagemService.criarQuartosEmLote(req.params.loteId, req.body, req.usuario!.id)); }
  catch (error: any) { return res.status(400).json({ erro: error.message || "Erro ao criar os quartos" }); }
});
