...
function generoLabel(valor: string) { return valor === "masculino" ? "Masculino" : "Feminino"; }
...
export class HospedagemService {
  ...
  static async criarQuartosEmLote(loteId: string, input: any, atorId: string) {
    ...
    if (!titulo) throw new Error("Informe um título para identificar o conjunto de quartos");
    if (configuracoes.length < 1 || configuracoes.length > 20) throw new Error("Inclua de 1 a 20 configurações de quartos");
    ...
    const normalizadas = configuracoes.map((item: any, indice: number) => {
      ...
      if (!Number.isInteger(quantidade) || quantidade < 1 || quantidade > 50) throw new Error(`Configuração ${indice + 1}: a quantidade de quartos deve ficar entre 1 e 50`);
      if (!Number.isInteger(capacidade) || capacidade < 1 || capacidade > 30) throw new Error(`Configuração ${indice + 1}: as vagas por quarto devem ficar entre 1 e 30`);
      if (!["masculino", "feminino"].includes(genero)) throw new Error(`Configuração ${indice + 1}: selecione masculino ou feminino`);
      if (!CLIMATIZACOES.includes(climatizacao)) throw new Error(`Configuração ${indice + 1}: climatização inválida`);
    ...
    return db.transaction(async (tx) => {
      ...
      for (const pacoteId of pacotes) {
        ...
        if (!pacote) throw new Error("Há uma configuração vinculada a pacote inválido para este período");
      }
      ...
      for (const config of normalizadas) {
        for (let i = 0; i < config.quantidade; i += 1) {
          ...
          const criado = linhas(await tx.execute(sql`INSERT INTO quartos_hospedagem (id, lote_id, pacote_id, nome, genero, capacidade, observacoes, ativo, criado_por, criado_em, atualizado_em)
            VALUES (${id}, ${loteId}, ${config.pacoteId}, ${nome}, ${config.genero}, ${config.capacidade}, ${observacoes || null}, true, ${atorId}, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) RETURNING *`))[0];
          await registrar(tx, "quarto", id, "quarto_criado_em_lote", atorId, undefined, { ...criado, titulo_lote: titulo, climatizacao: config.climatizacao });
          criados.push(criado);
        }
      }
      return { titulo, total: criados.length, quartos: criados };
    });
  }
