...
import { BedDouble, Layers3, MoveRight, Plus, RefreshCw, Trash2, UserMinus } from 'lucide-react';
...
type ConfiguracaoLote = {
  ...
  capacidade: string;
  climatizacao: 'ar_condicionado' | 'ventilador' | 'sem_climatizacao';
  pacote_id: string;
  observacoes: string;
};
const novaConfiguracao = (): ConfiguracaoLote => ({ quantidade: '1', genero: 'feminino', capacidade: '4', climatizacao: 'ar_condicionado', pacote_id: '', observacoes: '' });
...
export default function HospedagemQuartos() {
  ...
  const totalNovosQuartos = useMemo(() => configuracoes.reduce((total, item) => total + Math.max(0, Number(item.quantidade) || 0), 0), [configuracoes]);
  const totalNovasVagas = useMemo(() => configuracoes.reduce((total, item) => total + (Math.max(0, Number(item.quantidade) || 0) * Math.max(0, Number(item.capacidade) || 0)), 0), [configuracoes]);
  ...
  const abrirLote = () => { setTituloLote(''); setConfiguracoes([novaConfiguracao()]); setModalLote(true); };
  ...
  const salvarLote = async (event: React.FormEvent) => {
    ...
    const payload = {
      titulo: tituloLote,
      configuracoes: configuracoes.map((item) => ({ ...item, quantidade: Number(item.quantidade), capacidade: Number(item.capacidade) })),
    };
    const ok = await executar(() => api.post(`/hospedagem/lotes/${loteId}/quartos/lote`, payload), `${totalNovosQuartos} quarto(s) criado(s) com ${totalNovasVagas} vaga(s).`);
  ...
  return <div className="admin-page">
    <div className="admin-page-header"><div><p className="admin-eyebrow">Operação da hospedagem</p><h1 className="admin-title">Quartos e hóspedes</h1><p className="admin-subtitle">Cadastre vários quartos de uma vez e organize ocupação, grupos e remanejamentos.</p></div><div className="flex flex-wrap gap-2"><Button onClick={abrirLote} disabled={!loteId}><Layers3 size={16} className="mr-2" />Criar quartos em lote</Button><Button variant="outline" onClick={abrirNovo} disabled={!loteId}><Plus size={16} className="mr-2" />Quarto avulso</Button><Button variant="outline" onClick={() => void carregar()}><RefreshCw size={16} className="mr-2" />Atualizar</Button></div></div>
      ...
      <form onSubmit={salvarLote} className="space-y-5">
        ...
        <div className="space-y-4">{configuracoes.map((item, indice) => <section key={indice} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <label className="text-sm font-medium">Grupo<select className="admin-field mt-1" value={item.genero} onChange={(e) => atualizarConfiguracao(indice, 'genero', e.target.value)}><option value="feminino">Feminino</option><option value="masculino">Masculino</option></select></label>
            <label className="text-sm font-medium">Climatização<select className="admin-field mt-1" value={item.climatizacao} onChange={(e) => atualizarConfiguracao(indice, 'climatizacao', e.target.value)}><option value="ar_condicionado">Ar-condicionado</option><option value="ventilador">Ventilador</option><option value="sem_climatizacao">Sem climatização</option></select></label>
          </div>
          <div className="mt-3 grid gap-3 md:grid-cols-2"><label className="text-sm font-medium">Pacote específico (opcional)<select className="admin-field mt-1" value={item.pacote_id} onChange={(e) => atualizarConfiguracao(indice, 'pacote_id', e.target.value)}><option value="">Todos os pacotes do período</option>{pacotes.map((p) => <option key={p.id} value={p.id}>{p.nome}</option>)}</select></label><Input label="Observação (opcional)" value={item.observacoes} onChange={(e) => atualizarConfiguracao(indice, 'observacoes', e.target.value)} /></div>
        </section>)}</div>
        <Button type="button" variant="outline" onClick={() => setConfiguracoes((atual) => [...atual, novaConfiguracao()])}><Plus size={16} className="mr-2" />Incluir mais configuração</Button>
        <div className="rounded-xl bg-[#f4f8f9] p-4 text-sm text-slate-700"><strong>{totalNovosQuartos}</strong> quarto(s) serão criados, somando <strong>{totalNovasVagas}</strong> vaga(s). Cada quarto recebe ID automático e permanece editável individualmente depois.</div>
        <div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => setModalLote(false)}>Cancelar</Button><Button type="submit" disabled={salvando || !tituloLote.trim() || totalNovosQuartos < 1}>{salvando ? 'Criando...' : 'Criar quartos'}</Button></div>
      </form>
    </AdminModal>
