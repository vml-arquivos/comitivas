...
export default function Contratos() {
  ...
  const handleAprovar = async (contratoId: string) => {
    ...
    try {
      await api.post(`/admin/contratos/${contratoId}/aprovar`);
      setAcaoMsg('Contrato aprovado administrativamente; a cobrança agora pode ser liberada.');
      await carregar();
    } catch (err: any) {
  ...
  return (
    <div className="admin-page">
      ...
      <div className="admin-card flex flex-col gap-3 p-4 sm:flex-row">
        ...
        <select value={filtro} onChange={(e) => setFiltro(e.target.value as typeof filtro)} className={inputClass}>
      ...
      {contratoAtual && formulario && (
        <Card>
          <CardContent className="p-6">
            <form onSubmit={handleGerarContrato} className="space-y-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h2 className="font-semibold text-gray-900">Gerar contrato padrão — reserva {contratoAtual.reserva_id.substring(0, 8)}</h2>
                  <p className="mt-1 text-sm text-gray-600">O texto-base segue o documento oficial anexado. Edite somente os dados variáveis; ao gerar, eles serão registrados e qualquer alteração futura criará nova versão.</p>
                </div>
                <button type="button" onClick={fecharGerarContrato} className="text-gray-500 hover:text-gray-700" aria-label="Fechar formulário">
                  <X size={18} />
                </button>
              </div>
              ...
              <section className={sectionClass}>
                <h3 className="font-semibold text-gray-900">1. Qualificação das partes</h3>
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  {input('Nome completo', 'contratante', 'nome')}
                  {input('CPF', 'contratante', 'cpf', 'text', '000.000.000-00')}
                  {input('Data de nascimento', 'contratante', 'nascimento', 'date')}
                  {input('Telefone / WhatsApp', 'contratante', 'telefone')}
                  {input('E-mail', 'contratante', 'email', 'email')}
                  {input('Endereço completo', 'contratante', 'endereco')}
                </div>
              </section>

              <section className={sectionClass}>
                <h3 className="font-semibold text-gray-900">2. Hospedagem e serviços</h3>
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  {input('Check-in', 'hospedagem', 'check_in', 'date')}
                  {input('Check-out', 'hospedagem', 'check_out', 'date')}
                  {input('Local de hospedagem', 'hospedagem', 'local', 'text', 'Chácara Recanto Novo Encantado ou Santa Thereza')}
                  <div>
                    <label className="mb-1 block text-sm font-medium text-gray-700" htmlFor="modalidade-hospedagem">
                      Modalidade de hospedagem
                    </label>
                    <select id="modalidade-hospedagem" className={inputClass} value={formulario.hospedagem.modalidade} onChange={(e) => atualizarFormulario('hospedagem', 'modalidade', e.target.value)}>
                      <option value="">Conforme contratação</option>
                      <option value="camping">CAMPING</option>
                      <option value="quarto_ventilador">QUARTO COM VENTILADOR COMPARTILHADO</option>
                      <option value="quarto_ar_condicionado">QUARTO COM CLIMATIZADOR COMPARTILHADO</option>
                    </select>
                  </div>
                </div>
                <p className="text-xs text-gray-500">Os serviços padrão permanecem no contrato: hospedagem, café da manhã, almoço, Open Bar das 9h às 19h e translado interno entre a chácara e o Parque do Peão.</p>
              </section>

              <section className={sectionClass}>
                <div className="flex items-center justify-between gap-3">
                  <h3 className="font-semibold text-gray-900">3. Transporte rodoviário</h3>
                  <label className="flex items-center gap-2 text-sm font-medium text-gray-700">
                    <input type="checkbox" checked={formulario.transporte.rodoviario_incluido} onChange={(e) => atualizarFormulario('transporte', 'rodoviario_incluido', e.target.checked)} /> Incluir no contrato
                  </label>
                </div>
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  {input('Local de embarque', 'transporte', 'local_embarque', 'text', 'Endereço do embarque')}
                  {input('Ponto de referência', 'transporte', 'ponto_referencia')}
                  {input('Data da saída', 'transporte', 'data_saida', 'date')}
                  {input('Horário previsto da saída', 'transporte', 'horario_saida', 'time')}
                  {input('Data prevista para retorno', 'transporte', 'data_retorno', 'date')}
                  {input('Horário previsto do retorno', 'transporte', 'horario_retorno', 'time')}
                  <div>
                    <label className="mb-1 block text-sm font-medium text-gray-700" htmlFor="tipo-veiculo">
                      Tipo do veículo
                    </label>
                    <select id="tipo-veiculo" className={inputClass} value={formulario.transporte.veiculo} onChange={(e) => atualizarFormulario('transporte', 'veiculo', e.target.value)}>
                  ...
                  {input('Limite de bagagem principal (kg)', 'bagagem', 'limite_kg', 'number', 'Ex.: 23')}
                </div>
              </section>

              <section className={sectionClass}>
                <h3 className="font-semibold text-gray-900">4. Seguro de viagem</h3>
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  {input('Seguradora', 'seguro', 'seguradora')}
                  {input('Número da apólice', 'seguro', 'apolice')}
                  {input('Cobertura', 'seguro', 'cobertura')}
                  {input('Telefone de atendimento', 'seguro', 'telefone')}
                </div>
                <p className="text-xs text-gray-500">Se não houver seguro adicional, o contrato registrará expressamente a inexistência e aplicará apenas as coberturas obrigatórias.</p>
              </section>

              <section className={sectionClass}>
                <h3 className="font-semibold text-gray-900">5. Pagamento e uso de imagem</h3>
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div>
                    <label className="mb-1 block text-sm font-medium text-gray-700" htmlFor="forma-pagamento">
                      Forma de pagamento
                    </label>
                    <select
                      id="forma-pagamento"
                      ...
                      onChange={(e) => {
                        const forma = e.target.value as 'pix' | 'boleto';
                        setFormaPagamentoContrato(forma);
                        if (forma === 'pix') setParcelasContrato('1');
                      }}
                    >
                  ...
                  <Input label="Quantidade de parcelas" type="number" min={1} max={20} disabled={formaPagamentoContrato === 'pix'} value={formaPagamentoContrato === 'pix' ? '1' : parcelasContrato} onChange={(e) => setParcelasContrato(e.target.value)} />
                  <label className="flex items-center gap-2 text-sm font-medium text-gray-700 md:col-span-2">
                    <input type="checkbox" checked={formulario.uso_imagem.autorizado} onChange={(e) => atualizarFormulario('uso_imagem', 'autorizado', e.target.checked)} /> Autorizar uso de imagem por 3 anos para divulgação institucional
                  </label>
                </div>
              </section>

              <section className={sectionClass}>
                <h3 className="font-semibold text-gray-900">6. Observações específicas</h3>
                <textarea value={formulario.observacoes_especificas} onChange={(e) => setFormulario((atual) => (atual ? { ...atual, observacoes_especificas: e.target.value } : atual))} rows={3} maxLength={500} className="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" placeholder="Opcional. Será incluído na versão do contrato." />
              </section>

              <div className="flex flex-wrap gap-2">
                <Button type="button" variant="outline" disabled={carregandoPreview} onClick={() => void handlePreview()}>
                  {carregandoPreview ? 'Montando preview...' : 'Visualizar preview'}
                </Button>
                <Button type="submit" disabled={salvandoContrato}>
                  {salvandoContrato ? 'Gerando...' : 'Gerar e congelar contrato'}
                </Button>
                <Button type="button" variant="outline" onClick={fecharGerarContrato}>
                  Cancelar
                </Button>
              </div>
              {previewHtml && (
                <div className="rounded-xl border border-gray-300 bg-white p-2">
                  <div className="mb-2 flex items-center justify-between px-2 text-xs font-semibold uppercase tracking-wide text-gray-500">
                    <span>Preview do modelo oficial</span>
                    <span>Somente leitura</span>
                  </div>
                  <iframe title="Preview do contrato" srcDoc={previewHtml} sandbox="allow-same-origin" className="h-[720px] w-full rounded-lg border border-gray-200" />
                </div>
              )}
            </form>
          </CardContent>
                  ...
                  contratosFiltrados.map((c) => (
                    <tr key={c.reserva_id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        ...
                        {c.vendedor_nome && <div className="mt-1 text-xs text-primary">Vendedor: {c.vendedor_nome}</div>}
                        {c.operacao && (
                          <div className="text-xs text-gray-500">
                            {c.operacao.onibus_nome} · lugar {c.operacao.poltrona}
                      ...
                      <td className="space-x-2 whitespace-nowrap px-6 py-4 text-right">
                        {c.contrato_gerado ? (
                          <>
                            <button onClick={() => handleVisualizar(c.reserva_id, c.pdf_disponivel)} className="p-1 text-gray-500 transition-colors hover:text-primary" title="Visualizar contrato">
                              <Eye size={18} />
                            </button>
                            {c.pdf_disponivel && (
                          ...
                          <button onClick={() => abrirGerarContrato(c)} className="p-1 text-gray-500 transition-colors hover:text-primary" title="Abrir formulário do contrato padrão">
                            <FileSignature size={18} />
                          </button>
                        )}
                      </td>
