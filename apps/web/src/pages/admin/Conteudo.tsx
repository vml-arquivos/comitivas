...
export default function Conteudo() {
  ...
  const [foto, setFoto] = useState({ url: '', alt_text: '', legenda: '' });
  ...
  const salvarFoto = async (event: React.FormEvent) => {
    ...
    if (!eventoId || !foto.url || !foto.alt_text) return setErro('Selecione o evento e informe URL e texto alternativo da foto.');
    setSalvando(true); setErro('');
    try { await api.post('/admin/fotos', { evento_id: eventoId, url_foto: foto.url, alt_text: foto.alt_text, legenda: foto.legenda }); setFoto({ url: '', alt_text: '', legenda: '' }); setMensagem('Foto adicionada à galeria.'); await carregar(); }
  ...
  const remover = async (tipo: 'fotos' | 'videos', id: string) => {
    if (!confirm('Remover este conteúdo do site?')) return;
    try { await api.delete(`/admin/${tipo}/${id}`); await carregar(); } catch (err: any) { setErro(err.response?.data?.erro || 'Não foi possível remover o conteúdo.'); }
  };
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-2">
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><ImagePlus size={19} />Adicionar foto</CardTitle></CardHeader><CardContent><form onSubmit={salvarFoto} className="space-y-3"><Input label="URL ou caminho da foto" value={foto.url} onChange={(e) => setFoto({ ...foto, url: e.target.value })} placeholder="/images/gallery/foto.jpg ou https://..." /><Input label="Texto alternativo" value={foto.alt_text} onChange={(e) => setFoto({ ...foto, alt_text: e.target.value })} placeholder="Descrição acessível da imagem" /><Input label="Legenda" value={foto.legenda} onChange={(e) => setFoto({ ...foto, legenda: e.target.value })} /><Button type="submit" disabled={salvando || !eventoId}>Salvar foto</Button></form></CardContent></Card>
