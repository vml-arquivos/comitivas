...
export default function Conteudo() {
  ...
  const salvarFoto = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setFoto({ arquivo: null, alt_text: '', legenda: '' });
  ...
  const salvarVideo = async (event: React.FormEvent) => {
    ...
    setErro('');
    try {
      await api.post('/admin/videos', { evento_id: eventoId, ...video });
      setVideo({ url: '', titulo: '', descricao: '' });
    ...
    <div className="admin-page">
      <div className="admin-page-header">
        <div>
          ...
          <p className="admin-subtitle">Alimente o conteúdo público por evento, sem editar código ou fazer novo deploy.</p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Evento do conteúdo</CardTitle>
        </CardHeader>
        <CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={salvarFoto} className="space-y-3">
              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700" htmlFor="conteudo-foto">
                  Foto do dispositivo
                </label>
              ...
              <Button type="submit" disabled={salvando || !eventoId}>
                Anexar foto
              </Button>
            </form>
          </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2">
            {fotos.map((item) => (
              <article key={item.id} className="overflow-hidden rounded-xl border border-gray-200">
                <img src={item.url_foto} alt={item.alt_text || item.legenda || 'Foto da excursão'} className="h-40 w-full object-cover" />
                <div className="space-y-2 p-3">
                  <p className="text-xs font-semibold text-primary">{nomeEvento(item.evento_id)}</p>
                  <p className="text-sm text-gray-600">{item.legenda || item.alt_text}</p>
                  <Button type="button" variant="outline" onClick={() => void remover('fotos', item.id, item.evento_id)}>
            ...
            {videos.map((item) => (
              <article key={item.id} className="rounded-xl border border-gray-200 p-4">
                ...
                <a className="mt-1 block break-all text-xs text-blue-700 underline" href={item.url} target="_blank" rel="noreferrer">
                  {item.url}
                </a>
                <Button type="button" variant="outline" className="mt-3" onClick={() => void remover('videos', item.id)}>
                  <Trash2 size={14} className="mr-1" />
                  Remover
                </Button>
              </article>
