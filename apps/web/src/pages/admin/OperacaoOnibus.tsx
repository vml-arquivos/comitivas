...
export default function OperacaoOnibus() {
  ...
  const carregarPeriodos = async (loteId: string) => {
    ...
    const eventoId = lote?.evento_id || saidas.find((item) => item.lote_id === loteId)?.evento_id;
    if (!eventoId) { setPeriodos([]); return; }
    const resposta = await api.get(`/eventos/${eventoId}/periodos`);
    setPeriodos(resposta.data.periodos || []);
  };
  ...
  const abrirEdicaoOnibus = (onibus: any) => {
    ...
    setOnibusForm({
      nome: onibus.nome || "",
      evento_periodo_id: onibus.evento_periodo_id || mapa?.saida.evento_periodo_id || "",
    ...
    <div className="admin-page">
        ...
        <>
          ...
          <section className="admin-card flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button type="button" onClick={() => { setOnibusEditando(null); setOnibusForm({ nome: `Transporte ${(mapa?.onibus.length || 0) + 1}`, evento_periodo_id: mapa?.saida.evento_periodo_id || "", capacidade: "44", identificacao: "", placa: "", motorista_nome: "", motorista_telefone: "", responsavel_nome: "" }); setModal("onibus"); }}><BusFront size={16} className="mr-2" />Novo veículo</Button>
            </div>
          </section>
      ...
      <AdminModal aberto={modal === "saida"} titulo="Nova saída operacional" descricao="Vincule a operação a uma viagem e lote já cadastrados." fechar={() => setModal(null)} largura="ampla">
        <form onSubmit={criarSaida} className="grid gap-4 md:grid-cols-2">
          <div className="md:col-span-2"><label className="mb-1 block text-sm font-medium">Viagem e lote</label><select required value={saidaForm.lote_id} onChange={(e) => { const lote = lotesComEvento.find((item) => item.id === e.target.value); setSaidaForm({ lote_id: e.target.value, evento_periodo_id: "", nome: lote ? `${lote.evento_nome} — ${lote.nome}` : "", data_partida: dataParaInput(lote?.data_embarque), data_retorno: dataParaInput(lote?.data_retorno) }); void carregarPeriodos(e.target.value).catch(() => setPeriodos([])); }} className={inputClass}><option value="">Selecione</option>{lotesComEvento.map((lote) => <option key={lote.id} value={lote.id}>{lote.evento_nome} · {lote.nome}</option>)}</select></div>
          <div className="md:col-span-2"><label className="mb-1 block text-sm font-medium">Período da excursão compartilhado (opcional)</label><select value={saidaForm.evento_periodo_id} onChange={(e) => setSaidaForm({ ...saidaForm, evento_periodo_id: e.target.value })} className={inputClass} disabled={!saidaForm.lote_id || periodos.length === 0}><option value="">Todos os períodos do lote</option>{periodos.map((periodo) => <option key={periodo.id} value={periodo.id}>{periodo.nome} · {new Date(periodo.data_inicio).toLocaleDateString("pt-BR")} a {new Date(periodo.data_fim).toLocaleDateString("pt-BR")}</option>)}</select><p className="mt-1 text-xs text-slate-500">Os mesmos ônibus desta janela atendem Camping e pacotes com quarto. Não existe um ônibus separado por tipo de pacote.</p></div>
      ...
      <AdminModal aberto={modal === "onibus" || modal === "editar_onibus"} titulo={onibusEditando ? "Editar veículo" : "Novo veículo"} descricao="A capacidade pode aumentar ou diminuir, sem remover lugares ocupados ou bloqueados." fechar={() => { setModal(null); setOnibusEditando(null); }} largura="ampla">
        <form onSubmit={salvarOnibus} className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2"><label className="mb-1 block text-sm font-medium">Período atendido</label><select value={onibusForm.evento_periodo_id} onChange={(e) => setOnibusForm({ ...onibusForm, evento_periodo_id: e.target.value })} className={inputClass} disabled={periodos.length === 0}><option value="">Ônibus geral da saída</option>{periodos.map((periodo) => <option key={periodo.id} value={periodo.id}>{periodo.nome} · {new Date(periodo.data_inicio).toLocaleDateString("pt-BR")} a {new Date(periodo.data_fim).toLocaleDateString("pt-BR")}</option>)}</select><p className="mt-1 text-xs text-slate-500">O ônibus fica compartilhado entre todos os pacotes que usam esta mesma janela. Deixe geral para atender todos os períodos da saída.</p></div>
