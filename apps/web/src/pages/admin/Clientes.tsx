...
export default function Clientes() {
  ...
  const [filtroTipo, setFiltroTipo] = useState<string>(() => {
    const solicitado = searchParams.get('tipo');
    if (solicitado === 'dev' && user?.tipo !== 'dev') return 'cliente';
    return ['cliente', 'vendedor', 'admin', 'dev', ''].includes(solicitado || '') ? (solicitado || 'cliente') : 'cliente';
  });
  ...
  const handleExcluir = async (usuario: Usuario) => {
    if (!confirm(`Excluir ${usuario.nome}? Se houver histórico, o cadastro será arquivado para preservar contratos, reservas e pagamentos.`)) return;
    try {
      ...
      alert(err.response?.data?.erro || 'Não foi possível excluir ou arquivar este cadastro.');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-wide text-primary">Cadastros</p>
          <h1 className="text-3xl font-bold text-gray-900">Clientes</h1>
          <p className="mt-1 max-w-3xl text-sm text-gray-600">Consulte cadastros, abra a ficha completa e mantenha os dados atualizados.</p>
        </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      >
                        ...
                        {(user?.tipo === 'dev' || (editandoId && form.tipo === 'admin')) && <option value="admin">Administrador</option>}
                        {user?.tipo === 'dev' && <option value="dev">DEV</option>}
                      </select>
                    </div>
                    <Input
                  ...
                  <tr key={usuario.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                      ...
                      <button
                        ...
                        title="Excluir ou arquivar cadastro"
                        aria-label={`Excluir ou arquivar ${usuario.nome}`}
                      >
                        <Trash2 size={18} />
                      </button>
                    </td>
