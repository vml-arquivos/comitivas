...
export default function EquipeAcessos() {
  ...
  const salvarEdicao = async (event: React.FormEvent) => {
    ...
    try {
      await api.put(`/admin/usuarios/${editando.id}`, { nome: editando.nome, email: editando.email, telefone: editando.telefone, equipe_nome: editando.equipe_nome });
  ...
  return (
    <div className="admin-page">
      ...
      <AdminModal aberto={Boolean(editando)} titulo="Editar acesso" descricao="Atualize apenas os dados necessários deste membro." fechar={() => setEditando(null)}>
        {editando && <form onSubmit={salvarEdicao} className="grid gap-4 sm:grid-cols-2"><Input required label="Nome" value={editando.nome} onChange={(e) => setEditando({ ...editando, nome: e.target.value })} /><Input required label="E-mail" type="email" value={editando.email} onChange={(e) => setEditando({ ...editando, email: e.target.value })} /><Input label="Telefone" value={editando.telefone || ''} onChange={(e) => setEditando({ ...editando, telefone: e.target.value })} /><Input label="Equipe / grupo" disabled={editando.tipo !== 'vendedor'} value={editando.equipe_nome || ''} onChange={(e) => setEditando({ ...editando, equipe_nome: e.target.value })} /><div className="flex justify-end gap-2 sm:col-span-2"><Button type="button" variant="outline" onClick={() => setEditando(null)}>Cancelar</Button><Button type="submit" disabled={salvando}>Salvar</Button></div></form>}
      </AdminModal>
    </div>
  );
}
