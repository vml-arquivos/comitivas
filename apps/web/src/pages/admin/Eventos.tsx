          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            {pacotesAbertos === loteOperacional?.id && <div className="mt-4 flex flex-col border-t border-slate-100 pt-4"><div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><h4 className="font-bold text-slate-900">Pacotes desta excursão</h4><p className="mt-1 text-xs text-slate-500">Cada pacote tem seus períodos, formas de contratação, operação e condições comerciais.</p></div><Button variant="outline" onClick={() => { setPacoteEditando(null); setPacoteForm(vazioPacote); }}><Plus size={15} className="mr-2" />Criar novo pacote</Button></div>
              <form id={`config-pacote-${loteOperacional.id}`} onSubmit={(e) => void salvarPacote(e, loteOperacional.id)} className="rounded-2xl border border-[#C94F38]/20 bg-[#fffaf7] p-4 shadow-sm sm:p-5">
                <div className="mb-5 flex flex-col gap-3 border-b border-[#C94F38]/10 pb-4 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea value={pacoteForm.descricao} onChange={(e) => setPacoteForm({ ...pacoteForm, descricao: e.target.value })} rows={3} maxLength={2000} className="w-full rounded-xl border border-slate-300 bg-white p-3 text-sm" placeholder="Descreva o que o cliente encontrará neste pacote." />
                  </div>
                </div>

                <div className="mt-4 rounded-xl border border-[#C94F38]/15 bg-white p-3">
                  <p className="text-sm font-bold text-slate-800">Destaque comercial na vitrine <span className="font-normal text-slate-400">(opcional)</span></p>
                  <p className="mt-1 text-xs leading-5 text-slate-500">Use para uma chamada geral do pacote. Promoções, preços e vagas específicas ficam nos lotes comerciais.</p>
                  <div className="mt-3 grid gap-3 md:grid-cols-2"><Input label="Título do destaque" value={pacoteForm.destaqueTitulo} onChange={(e) => setPacoteForm({ ...pacoteForm, destaqueTitulo: e.target.value })} maxLength={160} placeholder="Ex.: Pré-venda aberta" /><Input label="Subtítulo" value={pacoteForm.destaqueSubtitulo} onChange={(e) => setPacoteForm({ ...pacoteForm, destaqueSubtitulo: e.target.value })} maxLength={255} placeholder="Ex.: Consulte os lotes disponíveis" /></div>
                  <label className="mt-3 block text-sm font-medium text-slate-700">Texto do destaque<textarea value={pacoteForm.destaqueTexto} onChange={(e) => setPacoteForm({ ...pacoteForm, destaqueTexto: e.target.value })} maxLength={4000} rows={2} className="mt-1 w-full rounded-md border border-slate-300 p-2 text-sm" placeholder="Ex.: Escolha seu período e condição de venda." /></label>
                </div>

                <div className="mt-5 rounded-xl border border-slate-200 bg-white p-3 sm:p-4">
                  <p className="text-sm font-bold text-slate-800">Condição de pagamento do pacote <span className="font-normal text-slate-400">(opcional nesta etapa)</span></p>
                  <p className="mt-1 text-xs leading-5 text-slate-500">Preço, quantidade de vagas e janela de venda ficam nos lotes comerciais. Estas regras de pagamento ficam salvas no pacote e não precisarão ser redigitadas ao editar.</p>
                  <details className="mt-3">
                    <summary className="cursor-pointer text-xs font-bold text-[#851F32]">Configurar formas e parcelamento</summary>
                    <div className="mt-3 grid gap-3 md:grid-cols-4">
                      <div className="md:col-span-4 flex flex-wrap gap-3 text-xs text-slate-600"><strong>Pagamento:</strong>{['pix', 'boleto', 'credito', 'debito'].map((forma) => <label key={forma} className="flex items-center gap-1"><input type="checkbox" checked={pacoteForm.formasPagamento.includes(forma)} onChange={(e) => setPacoteForm({ ...pacoteForm, formasPagamento: e.target.checked ? [...pacoteForm.formasPagamento, forma] : pacoteForm.formasPagamento.filter((item) => item !== forma) })} />{forma === 'pix' ? 'PIX' : forma === 'boleto' ? 'Boleto' : forma === 'credito' ? 'Cartão de crédito' : 'Cartão de débito'}</label>)}</div>
                      <Input label="Limite dos boletos" type="date" value={pacoteForm.dataLimitePagamento} onChange={(e) => setPacoteForm({ ...pacoteForm, dataLimitePagamento: e.target.value })} />
                      <Input label="Máx. parcelas boleto" type="number" min={1} max={36} value={pacoteForm.boletoParcelas} onChange={(e) => setPacoteForm({ ...pacoteForm, boletoParcelas: e.target.value })} />
                      <Input label="Máx. parcelas cartão" type="number" min={1} max={24} value={pacoteForm.creditoParcelas} onChange={(e) => setPacoteForm({ ...pacoteForm, creditoParcelas: e.target.value })} />
                      ...
                      <Input label="Juros cartão / mês (%)" type="number" min={0} max={20} step="0.01" value={pacoteForm.creditoJurosMensal} onChange={(e) => setPacoteForm({ ...pacoteForm, creditoJurosMensal: e.target.value })} />
                      ...
                      <Input label="Juros de mora / mês (%)" type="number" min={0} max={20} step="0.01" value={pacoteForm.jurosMoraMensal} onChange={(e) => setPacoteForm({ ...pacoteForm, jurosMoraMensal: e.target.value })} />
                    </div>
                  </details>
                </div>
