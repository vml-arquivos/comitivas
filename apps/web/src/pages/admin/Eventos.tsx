...
type PeriodoPacote = {
  ...
  data_embarque?: string | null;
  data_retorno?: string | null;
  capacidade_transporte_planejada?: number | null;
...
type FormaContratacaoConfiguravel = Exclude<FormaContratacao, 'livre'>;
...
interface Pacote {
  ...
  formas_contratacao?: FormaContratacaoConfiguravel[];
...
function formasConfiguradasLabel(pacote: Pacote) {
  const formas = pacote.formas_contratacao?.length ? pacote.formas_contratacao : [pacote.forma_contratacao];
  return formas.filter((forma): forma is FormaContratacao => Boolean(forma)).map((forma) => formaLabels[forma] || forma).join(' · ');
}
...
export default function EventosAdmin() {
  ...
  const carregarLotes = async (eventoId: string) => {
    ...
    let lista = (resposta.data.lotes || []) as Lote[];
    try {
      const estrutura = await api.post(`/eventos/${eventoId}/estrutura-operacional`);
      const lote = estrutura.data.lote as Lote | undefined;
      if (lote) lista = [lote, ...lista.filter((item) => item.id !== lote.id)];
    } catch {
      // A listagem já carregada continua utilizável mesmo se uma excursão
      // legada tiver uma estrutura inconsistente; não perde dados da tela.
    }
    setLotesPorEvento((atual) => ({ ...atual, [eventoId]: lista }));
  };
  ...
  const carregarLotesComerciais = async (pacoteId: string) => {
    ...
    setLotesComerciaisPorPacote((atual) => ({ ...atual, [pacoteId]: resposta.data.lotes || [] }));
  };
  ...
  const salvarEvento = async (e: FormEvent) => {
    ...
    try {
      ...
      setMensagem(eventoEditando ? 'Excursão atualizada.' : 'Excursão criada. Agora configure os pacotes e períodos.');
  ...
  const salvarPacote = async (e: FormEvent, loteId: string) => {
    ...
    try {
      ...
      const payload = {
        ...
        onibus_config: pacoteForm.formasContratacao.some((forma) => forma.includes('onibus')) ? Array.from({ length: Math.max(1, Number(pacoteForm.quantidadeOnibus) || 1) }, (_, indice) => ({ id: `onibus-${indice + 1}`, nome: `Ônibus ${indice + 1}`, capacidade: Math.max(1, Number(pacoteForm.capacidadeOnibus) || 1) })) : [],
        configuracao_pagamento: {
        ...
        destaque_titulo: pacoteForm.destaqueTitulo.trim() || null,
        destaque_subtitulo: pacoteForm.destaqueSubtitulo.trim() || null,
        destaque_texto: pacoteForm.destaqueTexto.trim() || null,
      };
  ...
  const salvarLoteComercial = async (e: FormEvent, pacote: Pacote) => {
    e.preventDefault();
    limparFeedback();
    const periodos = periodosPorPacote[pacote.id] || pacote.periodos || [];
    if (periodos.length > 0 && !loteComercialForm.periodoId) { setErro('Selecione o período deste lote comercial.'); return; }
    if (!loteComercialForm.nome.trim() || !loteComercialForm.vagas || !loteComercialForm.valor || !loteComercialForm.dataInicio) { setErro('Informe nome, vagas, preço e início da venda.'); return; }
    if (loteComercialForm.dataFim && loteComercialForm.dataInicio > loteComercialForm.dataFim) { setErro('O início da venda deve ser anterior ao fim.'); return; }
    const vagas = Number(loteComercialForm.vagas);
    const valor = Number(loteComercialForm.valor);
    if (!Number.isInteger(vagas) || vagas < 1 || !Number.isFinite(valor) || valor < 0) { setErro('Informe vagas inteiras e um preço válido.'); return; }
    setSalvando(true);
    try {
      const payload = {
      ...
      await carregarLotesComerciais(pacote.id);
      setLoteComercialForm(vazioLoteComercial); setLoteComercialEditando(null);
      setMensagem(loteComercialEditando ? 'Lote comercial atualizado.' : 'Lote comercial criado.');
    } catch (err: any) { setErro(erroDaApi(err, 'Não foi possível salvar o lote comercial.')); }
    finally { setSalvando(false); }
  };
  ...
  return <div className="admin-page">
    <section className="admin-page-header"><div className="flex w-full flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><p className="admin-eyebrow">Catálogo</p><h1 className="admin-title">Excursões e pacotes</h1><p className="admin-subtitle">Crie a excursão, depois configure pacotes, períodos, operação e lotes comerciais por etapa.</p></div><Button onClick={() => { limparFeedback(); setEventoEditando(null); setEventoForm(vazioEvento); setMostrarFormEvento((atual) => !atual); }}>{mostrarFormEvento ? <X size={16} className="mr-2" /> : <Plus size={16} className="mr-2" />}{mostrarFormEvento ? 'Fechar' : 'Nova excursão'}</Button></div></section>
    ...
    <div className="space-y-4">{eventos.map((evento) => {
      ...
      const loteOperacional = lotes.find((lote) => lote.operacional_interno) || lotes[0];
