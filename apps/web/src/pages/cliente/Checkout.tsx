...
export default function Checkout() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        const response = await api.post(`/pacotes/reservas/${reservaId}/simular-pagamento`, { metodo_pagamento: metodoPagamento, quantidade_parcelas: metodoPagamento === 'pix' ? 1 : quantidadeParcelas });
  ...
  const resumo = useMemo(() => {
    ...
    return { base, desconto: Number(simulacao?.condicao_pagamento?.desconto_pagamento ?? desconto), taxa: Number(simulacao?.condicao_pagamento?.taxa_pagamento || 0), juros: Number(simulacao?.condicao_pagamento?.juros_pagamento || 0), total, quantidade, parcela: Number(simulacao?.condicao_pagamento?.valor_parcela ?? total / quantidade) };
