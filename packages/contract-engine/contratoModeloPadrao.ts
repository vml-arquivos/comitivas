...
export function renderizarContratoModeloPadrao({ snapshot, reservaId }: ContratoModeloContext): string {
  ...
  const custosAntecipados = temHospedagem
    ? "hospedagem, fornecedores, alimentação, equipe e estrutura operacional"
    : "transporte, fornecedores, equipe e estrutura operacional";
  ...
  const adicionais = (Array.isArray(snapshot.adicionais) ? snapshot.adicionais : [])
    .map((item) => ({ nome: String(item?.nome || "").trim(), quantidade: Math.max(1, Number(item?.quantidade || 1)) }))
  ...
  const objetoTexto = temHospedagem && t.rodoviario_incluido
    ...
    : temHospedagem
      ...
      : t.rodoviario_incluido
        ? `O presente contrato tem por objeto o transporte rodoviário do pacote ${escapeHtml(pacoteNome)}, para ${escapeHtml(eventoNome)}, em ${escapeHtml(eventoLocal)}, sem contratação de hospedagem.`
        : `O presente contrato compreende somente os serviços expressamente descritos no pacote ${escapeHtml(pacoteNome)}.`;
...
<div class="check-options">${modality(modalidade, "quarto_ventilador", "QUARTO COM VENTILADOR COMPARTILHADO.")}${modality(modalidade, "quarto_ar_condicionado", "QUARTO COM CLIMATIZADOR COMPARTILHADO.")}</div>
...
<p class="clause"><strong>3.3</strong> Os quartos são compartilhados e separados por grupo masculino e feminino, respeitada a capacidade cadastrada para cada quarto.</p>
<p class="clause"><strong>3.4</strong> A estrutura e os serviços da hospedagem são os descritos no pacote escolhido.</p>
<p class="clause"><strong>3.5</strong> A roupa de cama e os itens de higiene pessoal são de responsabilidade do hóspede, salvo disposição expressa em contrário.</p>` : "";
  const blocoDanos = temHospedagem
    ? `<p><strong>9.1.</strong> O <strong>CONTRATANTE</strong> compromete-se a zelar pelas instalações da hospedagem, áreas comuns, equipamentos, mobiliários e demais bens disponibilizados durante a execução do contrato.</p>
<p><strong>9.2.</strong> Todo dano material causado pelo <strong>CONTRATANTE</strong> às instalações ou aos bens disponibilizados será de sua exclusiva responsabilidade, obrigando-se ao ressarcimento integral dos prejuízos efetivamente apurados.</p>`
    : `<p><strong>9.1.</strong> O <strong>CONTRATANTE</strong> compromete-se a zelar pelos veículos, equipamentos e demais bens disponibilizados durante a execução do contrato.</p>
<p><strong>9.2.</strong> Todo dano material causado pelo <strong>CONTRATANTE</strong> aos bens disponibilizados será de sua exclusiva responsabilidade, obrigando-se ao ressarcimento integral dos prejuízos efetivamente apurados.</p>`;
...
<p><strong>II – Parcelamento por boleto bancário:</strong> sem incidência de juros, observado que a quantidade de parcelas disponíveis será definida de acordo com a data da contratação, devendo o valor integral do pacote estar obrigatoriamente quitado antes do ${escapeHtml(marcoServico)}.<br/>Atentando-se as seguintes datas:</p>
...
<p><strong>7.2.</strong> Considerando que o pacote para ${escapeHtml(eventoNome)} demanda reservas antecipadas de ${escapeHtml(custosAntecipados)}, o cancelamento da contratação sujeitará o <strong>CONTRATANTE</strong> às retenções abaixo especificadas, destinadas exclusivamente à compensação das despesas administrativas, operacionais e financeiras já assumidas pela <strong>CONTRATADA.</strong></p>
...
<p>I – Cancelamento realizado com antecedência superior a 90 (noventa) dias do ${escapeHtml(marcoServico)}: retenção de 10% (dez por cento) do valor total contratado;<br/>II – Cancelamento realizado entre 80 (oitenta) e 60 (sessenta) dias antes do ${escapeHtml(marcoServico)}: retenção de 20% (vinte por cento);<br/>III – Cancelamento realizado entre 50 (cinquenta) e 30 (trinta) dias antes do ${escapeHtml(marcoServico)}: retenção de 30% (trinta por cento);<br/>IV – Cancelamento realizado entre 20 (vinte) e 15 (quinze) dias antes do ${escapeHtml(marcoServico)}: retenção de 50% (cinquenta por cento);<br/>V – Cancelamento realizado com menos de 15 (quinze) dias de antecedência ao ${escapeHtml(marcoServico)}: retenção de 80% (oitenta por cento) do valor total contratado, considerando a elevada dificuldade de reposição da vaga e as despesas operacionais já assumidas pela <strong>CONTRATADA.</strong><br/>VI – Na hipótese de não comparecimento do <strong>CONTRATANTE</strong> no ${escapeHtml(marcoServico)} (no-show), sem comunicação prévia, ou de abandono voluntário após o início da prestação dos serviços, haverá retenção de 100% (cem por cento) do valor contratado, não sendo devido qualquer reembolso, em razão da efetiva disponibilização da vaga e da impossibilidade de sua comercialização a terceiros.</p>
...
${temHospedagem ? `<h2>CLÁUSULA DÉCIMA OITAVA<br/>DO TRANSLADO</h2>
<p>O translado compreende exclusivamente o percurso entre a hospedagem e o Parque do Peão, em horários previamente divulgados pela organização.</p>` : ""}
